...
def predecir_produccion(
    ...
    if modelo_tipo == "linear":
        ...
        raise ValueError(
            "Modelo no válido. Use 'linear' o 'ridge'"
        )
...
def generar_caso_de_uso_produccion():
    ...
    toneladas = np.random.uniform(
        800,
        3000,
        n
    )

    horas = np.random.uniform(
        16,
        24,
        n
    )

    eficiencia = np.random.uniform(
        70,
        98,
        n
    )
    ...
    mantenimiento = np.random.randint(
        1,
        6,
        n
    )
    ...
    metricas, coeficientes, modelo = predecir_produccion(
        df,
